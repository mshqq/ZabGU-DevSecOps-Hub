...
CI проверяет линт, форматирование и тесты — прогони их локально, чтобы не ловить красный статус:

```bash
ruff check .            # линт
ruff format .           # форматирование (без --check - применяет правки)
```

Если `ruff format` без `--check` изменил файлы - просто закоммить результат, это ожидаемо.
